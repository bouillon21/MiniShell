...
int	asis_export(t_all *all, int i, char *line, char **tmp)
{
	...
	else if (ft_strnstr(all->token->args[i], "+=",
			ft_strlen(all->token->args[i])))
	{
		tmp = separation_line(all->token->args[i]);
		if (env_srh(all, tmp[0])
			&& env_srh(all, tmp[0])->content->value != NULL)
		{
...
void	export(t_all *all)
{
	...
	else
	{
		...
		{
			if (asis_export(all, i, line, tmp))
			{
				if (ft_strchr(all->token->args[i], '=') != NULL)
				{
					tmp = separation_line(all->token->args[i]);
					env_add(all, tmp[0], tmp[1]);
					free_array(&tmp);
